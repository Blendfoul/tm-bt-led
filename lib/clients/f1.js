...
class F1 extends AbstractClient {
  ...
  startClient = () => {
    ...
    const showTyreTemp = (onRight) => {
      this.tmBtLed.setTemperature(
        carTelemetry.m_tyresSurfaceTemperature.reduce((a, b) => a + b, 0) /
          carTelemetry.m_tyresSurfaceTemperature.length || 0,
        onRight,
      );
    };
    ...
    const showBrakeTemp = (onRight) => {
      this.tmBtLed.setTemperature(
        carTelemetry.m_brakesTemperature.reduce((a, b) => a + b, 0) /
          carTelemetry.m_brakesTemperature.length || 0,
        onRight,
      );
    };
    ...
    this.client.on(PACKETS.lapData, (d) => {
      ...
      const fractionOfLap =
        trackLength === 0 || lapData.m_lapDistance < 0
          ? 0
          : (lapData.m_lapDistance / trackLength).toFixed(6);
      if (fractionOfLap >= 0.997 && currentDeltaData.size > 1000) {
        if (
          lapData.m_currentLapInvalid !== 1 &&
          (lapData.m_currentLapTime <= bestLapTime || !bestLapTime)
        ) {
          bestDeltaData.clear();
    ...
    this.client.on(PACKETS.carTelemetry, (d) => {
      ...
      if (this.tmBtLed.revLightsFlashing !== 1) {
        // No override because of pit limiter
        if (modernCar) {
