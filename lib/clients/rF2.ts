...
// @ts-expect-error - rF2SharedMemory is not a module
import rF2SharedMemory from '../../rF2SharedMemory/build/Release/rF2SharedMemory.node';
import AbstractClient from '../abstractClient';
import path from 'path';
import { TmBTLed } from '../tmBTLed';

const loadableConfigName = 'rf2.config.jsonc';
const defaultConfig = {
...
class rF2 extends AbstractClient {
  ...
  showLastLap = (onRight: boolean) => {
    this.tmBtLed.setTime(
      (this.scoring.mLastLapTime < 0 ? 0 : this.scoring.mLastLapTime) * 1000,
      onRight,
    );
  };
  showBestLap = (onRight: boolean) => {
    this.tmBtLed.setTime(
      (this.scoring.mBestLapTime < 0 ? 0 : this.scoring.mBestLapTime) * 1000,
      onRight,
    );
  };
