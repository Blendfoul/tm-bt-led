...
class Game extends AbstractClient {
  ...
  keys = ['speed', 'rpm', 'gear', 'max_rpm', 'current_lap_time', 'position'];
   ...
   * @param message
   */
  parseData = (message: Uint8Array) => {
    // Debug
    console.log('Received message ', message);

    // Transform received udp data to structured data (game specific)
    const data = this.transformData<Data>(message);
    ...
    switch (this.currentLeftMode) {
      ...
      case 0: // SPD
        ...
        this.tmBtLed.setSpeed((data['speed'] * 3.6).toString());
        break;
      case 1: // RPM
   ...
   * @return parsedData
   */
  transformData = <T extends object = {}>(message: Uint8Array) => {
    const view = new DataView(message.buffer);
    const parsedData = {} as T;
    let parsedKeys = 0;
    for (let i = 0; i < message.length; i++) {
      parsedData[this.keys[parsedKeys++] as keyof T] = view.getFloat32(
        i,
        true,
